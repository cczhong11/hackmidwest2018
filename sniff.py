	...
	if pkt.type == 0 and pkt.subtype == 4: #subtype used to be 8 (APs) but is now 4 (Probe Requests)
		...
		if curmac not in IGNORE_LIST: #If not registered as ignored
			if curmac in d:
				...
				print('\033[95m'#!/usr/bin/env python
...
sniff(iface="mon0", prn = PacketHandler) + 'Probe MAC Address: ' + pkt.addr2 + ' from device ' + '\033[93m' + d[curmac] + '\033[0m')
...
def main():
	...
	parser.add_argument('--interface', '-i', default='en0', # Change mon0 to your monitor-mode enabled wifi interface
